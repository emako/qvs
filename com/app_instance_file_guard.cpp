#include "app_instance_file_guard.h"
...
AppInstanceFileGuard::AppInstanceFileGuard(const QString & a_fileName)
{
...
AppInstanceFileGuard::~AppInstanceFileGuard()
{
    if(m_file.isOpen())
        unlock();
}

bool AppInstanceFileGuard::lock(const QString & a_fileName)
{
    if(m_file.isOpen())
    {
...
bool AppInstanceFileGuard::unlock()
{
    if(m_file.isOpen())
    {
#ifndef Q_OS_WIN
...
bool AppInstanceFileGuard::isLocked() const
{
    return m_file.isOpen();
}

QString AppInstanceFileGuard::error() const
{
    return m_error;
}
